...
def get_lr(it):
    ...
    coeff = 0.5 * (1.0 + math.cos(math.pi * decay_ratio))  # Fixed: cosine schedule should be 0.5 * (1 + cos(...))
...
for step in range(max_steps+1):
    ...
    with torch.autocast(device_type=device, dtype=torch.bfloat16):
        loss = model(x, labels=y)  # Fixed: pass y as labels
    loss.backward()
    norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)  # Fixed: clip_grad_norm_ (underscore at end)
    lr = get_lr(step)
